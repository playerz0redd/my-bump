...
protocol CloudStorageManagerProtocol {
    func uploadImage(image: Data) async throws(CloudStorageError) -> String
            ...
            return try await withCheckedThrowingContinuation { continuation in
                manager.createUploader().upload(data: image, uploadPreset: uploadPreset)
                    .response { result, error in
                        if let error = error {
                            continuation.resume(throwing: CloudStorageError.apiError(error))
                        }
                        else if let result = result, let url = result.publicId {
                            continuation.resume(returning: url)
                        }
                        else {
                            continuation.resume(throwing: CloudStorageError.unknownError)
                        }
                    }
            }
